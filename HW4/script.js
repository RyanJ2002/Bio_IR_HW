...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners() {
        ...
        // HW4 ======
        if (semanticToggle) {
            semanticToggle.addEventListener('change', () => performGlobalSearch(mainSearchInput.value));
        }
        // ======
    ...
    function getPlainTextAbstractFromXmlDoc(xmlDoc) {
        // 1. Try standard AbstractText (most common)
        const sections = xmlDoc.querySelectorAll('AbstractText');
        if (sections.length > 0) {
            return Array.from(sections).map(s => s.textContent.trim()).join(' ');
        }
        // 2. Try the Abstract tag directly
        const abstractNode = xmlDoc.getElementsByTagName('Abstract')[0];
        if (abstractNode) return abstractNode.textContent.trim();
    ...
    // HW4 ======
    function calculateGlobalIdfMap(allFiles) {
        const docFreq = {};
        ...
        allFiles.forEach(file => {
            ...
            const text = `${getTitleFromXmlDoc(xmlDoc)} ${getPlainTextAbstractFromXmlDoc(xmlDoc)}`;

            // Count unique words in this document
            const uniqueWords = new Set(processText(text, 'original'));
            uniqueWords.forEach(word => {
                docFreq[word] = (docFreq[word] || 0) + 1;
            });
        });

        // Calculate IDF: log( Total / DF )
        const idfMap = {};
        for (const [word, df] of Object.entries(docFreq)) {
            idfMap[word] = Math.log10(totalDocs / (df + 1)) + 1;
        }
        return idfMap;
    }


    function extractBestSentence(text, idfMap) {
        ...
        // If only one sentence (or it's just the title), return it immediately
        if (sentences.length === 1) return sentences[0].trim();

        const allWords = processText(text, 'original');
        const totalWordsCount = allWords.length || 1;
        const termFreq = {};
        allWords.forEach(w => termFreq[w] = (termFreq[w] || 0) + 1);
        ...
        sentences.forEach(sentence => {
            ...
            // Ignore fragments unless it's the only option
            if (cleanSentence.length < 10 && sentences.length > 1) return;

            const sentenceWords = processText(cleanSentence, 'original');
            if (sentenceWords.length === 0) return;

            let sumTfIdf = 0;
            sentenceWords.forEach(word => {
                const tf = (termFreq[word] || 0) / totalWordsCount;
                const idf = idfMap[word] || 1.5;
                sumTfIdf += (tf * idf);
            });

            const avgScore = sumTfIdf / sentenceWords.length;
    ...
    async function performGlobalSearch(query) {
        ...
        const useSemantic = typeof semanticToggle !== 'undefined' ? semanticToggle.checked : false;

        // --- 1. Pre-calculate Global IDF Map ---
        const globalIdfMap = calculateGlobalIdfMap(storedFiles);

        // --- Semantic Search Fetch ---
        let semanticTerms = [];
        if (useSemantic && lowerCaseQuery.length > 2) {
            try {
                const response = await fetch(`http://127.0.0.1:5000/get-all-similarities?keyword=${encodeURIComponent(lowerCaseQuery)}`);
                const data = await response.json();

                if (data.skipgram_no_stopwords) {
                    semanticTerms = data.skipgram_no_stopwords
                        .filter(i => i.score > 0.6)
                        .map(i => i.word.toLowerCase());

                    // Console Log for Debugging
                    console.groupCollapsed(`🔍 Semantic Analysis for "${lowerCaseQuery}"`);
                    console.log("AI Expanded Synonyms:", semanticTerms);
                    console.groupEnd();
                }
        ...
        storedFiles.forEach(file => {
            ...
            const matchedSemantics = new Set();
            const processedQuery = algorithm === 'porter' ? stemmer(lowerCaseQuery) : lowerCaseQuery;

            if (lowerCaseQuery === '') {
                isMatch = true;
            } else {
                const contentWords = processText(content, 'original');
                ...
                for (const word of uniqueContentWords) {
                    ...
                    if (levenshteinDistance(processedWord, processedQuery) <= tolerance) {
                        matchedOriginals.add(word);
                    }
                    if (useSemantic && semanticTerms.includes(word)) {
                        matchedSemantics.add(word);
                    }
                }

                if (matchedOriginals.size > 0 || matchedSemantics.size > 0) {
                    isMatch = true;
                    if (matchedSemantics.size > 0) {
                        console.log(`📄 Document: "${title.substring(0, 30)}..." matched AI words:`, Array.from(matchedSemantics));
                    }
                    const allMatches = [...matchedOriginals, ...matchedSemantics];
                    const matchRegex = new RegExp(`\\b(${allMatches.join('|')})\\b`, 'gi');
            ...
            if (isMatch) {
                // Snippet Calculation
                let snippetText = abstract;
                if (!snippetText || snippetText.length < 15) snippetText = title;
                const bestSentence = extractBestSentence(snippetText, globalIdfMap);
                ...
                currentSearchResults.push({
                    ...
                    matchedWords: Array.from(matchedOriginals),
                    semanticWords: Array.from(matchedSemantics),
                    authors: getAuthorsFromXmlDoc(xmlDoc),
                    pubDate: getPubDateFromXmlDoc(xmlDoc),
                    smartSnippet: bestSentence
                });
            }
        });

        // Sorting Logic
        if (sortBy === 'tfidf_density' && currentSearchResults.length > 0) {
            calculateTfidfSortScores(currentSearchResults, storedFiles, algorithm);
            currentSearchResults.sort((a, b) => b.densityScore - a.densityScore);
        } else if (sortBy === 'relevance' && lowerCaseQuery !== '') {
        ...
        // --- RESTORED: Show/Hide the Analyze Button ---
        // Only show if we have results AND the user has typed a query
        analyzeBtn.classList.toggle('hidden', currentSearchResults.length === 0 || lowerCaseQuery === '');
    }

    function renderSearchResults(results, query) {
        resultsArea.innerHTML = '';
        const currentSortMode = document.getElementById('home-sort-select').value;

        if (results.length === 0) {
            resultsArea.innerHTML = `<div class="placeholder-text"><p>找不到符合條件的文件。</p></div>`;
            return;
        }
        ...
        results.forEach((result, index) => {
            ...
            if (result.semanticWords && result.semanticWords.length > 0) {
                const semRegex = new RegExp(`\\b(${result.semanticWords.join('|')})\\b`, 'gi');
                highlightedTitle = highlightedTitle.replace(semRegex, `<mark class="semantic-match" title="AI Match">$&</mark>`);
            }
            if (query && result.matchedWords && result.matchedWords.length > 0) {
                const mainRegex = new RegExp(`\\b(${result.matchedWords.join('|')})\\b`, 'gi');
                highlightedTitle = highlightedTitle.replace(mainRegex, `<mark>$&</mark>`);
            }

            let statHtml = '';
            if (currentSortMode === 'tfidf_density') {
                const scoreDisplay = result.densityScore ? (result.densityScore * 1000).toFixed(2) : '0';
                statHtml = `<div class="stat-item"><span class="label">Topic Density</span><span class="value">${scoreDisplay}</span></div>`;
            } else {
                statHtml = `<div class="stat-item"><span class="label">搜尋詞筆數</span><span class="value">${result.keywordCount}</span></div>`;
            }

            const aiLabel = (result.matchedWords.length === 0 && result.semanticWords && result.semanticWords.length > 0)
                ? '<span style="font-size:0.8em; background:#d1e7dd; color:#0f5132; padding:2px 6px; border-radius:4px; margin-left:10px;">AI Match</span>'
                : '';

            // --- SNIPPET GENERATION ---
            let snippetHtml = '';

            // Check if it exists and is not just empty whitespace
            if (result.smartSnippet && result.smartSnippet.trim().length > 0) {
                let processedSnippet = result.smartSnippet;
                if (query && result.matchedWords.length > 0) {
                    const highlightRegex = new RegExp(`\\b(${result.matchedWords.join('|')})\\b`, 'gi');
                    processedSnippet = processedSnippet.replace(highlightRegex, `<mark>$&</mark>`);
                }

                // Optional: Add a label if it came from the Title fallback
                const labelText = (result.snippetSource === 'title') ? "AI Insight (from Title)" : "AI Insight";

                snippetHtml = `
                    <div class="smart-snippet" style="margin-top:8px; padding:8px 12px; background:#f8f9fa; border-left:3px solid #6c757d; font-style:italic; color:#555; font-size: 0.9rem;">
                        <strong><i class="fas fa-quote-left"></i> ${labelText}:</strong> ${processedSnippet}
                    </div>
                `;
            }
            ...
            <div class="info">
                <h3>${highlightedTitle} ${aiLabel}</h3>
                <p class="meta-line">
                    <i class="fas fa-users"></i> <strong>Authors:</strong> ${result.authors}
                </p>
                <p class="meta-line">
                    <i class="fas fa-calendar-alt"></i> <strong>Date:</strong> ${result.pubDate} | <strong>PMID:</strong> ${result.pmid}
                </p>
    ...
    function showPaperDashboard(fileTitle) {
        ...
        // --- 1. Generate Base HTML (with Section Labels if available) ---
        let htmlAbstract = "";
        const abstractSections = xmlDoc.querySelectorAll('Abstract > AbstractText');

        if (abstractSections.length > 0) {
            htmlAbstract = Array.from(abstractSections).map(section => {
                const label = section.getAttribute('Label');
                return label ? `<strong>${label}</strong><br>${section.textContent}` : section.textContent;
            }).join('<br><br>');
        } else {
            // Fallback for simple <Abstract> tag
            const simpleAbstract = xmlDoc.getElementsByTagName('Abstract')[0];
            htmlAbstract = simpleAbstract ? simpleAbstract.textContent : "No abstract available.";
        }

        const dashboardAbstractEl = document.getElementById('dashboard-abstract');
        const currentQuery = mainSearchInput.value.trim().toLowerCase();

        // --- 2. INTELLIGENT HIGHLIGHTING LOGIC ---

        // Try to find the pre-calculated result for this file
        // This contains the Semantic Matches found by performGlobalSearch
        const searchResult = currentSearchResults.find(r => r.storageTitle === fileTitle);

        let finalHtml = htmlAbstract;
        let count = 0;

        if (searchResult) {
            // Case A: We have search data (Semantic + Original)

            // 1. Highlight Semantic Matches (Green)
            if (searchResult.semanticWords && searchResult.semanticWords.length > 0) {
                const semRegex = new RegExp(`\\b(${searchResult.semanticWords.join('|')})\\b`, 'gi');
                finalHtml = finalHtml.replace(semRegex, `<mark class="semantic-match" title="AI Match">$&</mark>`);
            }

            // 2. Highlight Direct Matches (Yellow) 
            // Applied second so they overlay semantic ones if there's an overlap
            if (searchResult.matchedWords && searchResult.matchedWords.length > 0) {
                const mainRegex = new RegExp(`\\b(${searchResult.matchedWords.join('|')})\\b`, 'gi');
                finalHtml = finalHtml.replace(mainRegex, `<mark>$&</mark>`);
            }

            count = searchResult.keywordCount;

        } else if (currentQuery) {
            // Case B: Fallback (e.g., opened from Library View without a main search)
            // We can only do standard matching here because we don't have the API data
            const tolerance = parseInt(toleranceSelect.value, 10);
            ...
            if (matchedWords.size > 0) {
                ...
                finalHtml = finalHtml.replace(matchRegex, `<mark>$&</mark>`);
            }
        }

        // --- 3. Update DOM ---
        dashboardAbstractEl.innerHTML = finalHtml;

        // Show/Hide Stats Box
        if (currentQuery && (searchResult || count > 0)) {
            if (activeKeywordDisplay) activeKeywordDisplay.textContent = `"${currentQuery}"`;
            if (statKeywordCount) statKeywordCount.textContent = count;
            if (keywordStatDiv) keywordStatDiv.style.display = 'block';
        } else {
            if (keywordStatDiv) keywordStatDiv.style.display = 'none';
        }
    ...
    // HW4 ======
    function calculateTfidfSortScores(searchResults, allFiles, algorithm) {
        // 1. Calculate Document Frequency (DF) for the entire library (for IDF)
        const docFreq = {};
        const totalDocs = allFiles.length;
        ...
        allFiles.forEach(file => {
            ...
            const words = new Set(processText(text, algorithm));
            words.forEach(word => {
                docFreq[word] = (docFreq[word] || 0) + 1;
            });
        });

        // 2. Calculate Term Frequency (TF) for the "Super-Document" (the search results)
        const superDocFreq = {};
        let totalWordsInSuperDoc = 0;

        // We first need the full text of the results to build the Super-Doc
        // We can iterate the searchResults, but we need to fetch their content from allFiles
        searchResults.forEach(result => {
            const file = allFiles.find(f => f.title === result.storageTitle);
            if (!file) return;
            ...
            const words = processText(text, algorithm);

            // Save words to the result object temporarily so we don't have to re-process later
            result._tempWords = words;

            words.forEach(word => {
                superDocFreq[word] = (superDocFreq[word] || 0) + 1;
                totalWordsInSuperDoc++;
            });
        });

        // 3. Pre-calculate the Weight (TF-IDF) for every word found in the results
        const wordWeights = {};
        for (const [word, count] of Object.entries(superDocFreq)) {
            const tf = count / totalWordsInSuperDoc; // TF of Super-Doc

            // Standard IDF: log( Total / (DF + 1) ) + 1
            const df = docFreq[word] || 0;
            const idf = Math.log10(totalDocs / (df + 1)) + 1;

            wordWeights[word] = tf * idf;
        }

        // 4. Calculate the Average Score for each document
        searchResults.forEach(result => {
            const words = result._tempWords || [];
            if (words.length === 0) {
            ...
            let sumScore = 0;
            words.forEach(word => {
                sumScore += (wordWeights[word] || 0);
            });

            // The Mutation: Average TF-IDF
            result.densityScore = sumScore / words.length;
        });
    }
    // ======
